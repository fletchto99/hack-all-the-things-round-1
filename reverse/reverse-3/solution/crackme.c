...
void msg(int msg) {
    if (msg == 1) {
      printf("Still going! E-Corp servers under intense load!\n");
    } else if(msg == 2) {
      printf("Have a coffee, on E-Corp while we run this program\n");
    } else if (msg == 3) {
      printf("I think this should be done by the end of the year?\n");
    } else if (msg == 4) {
      printf("Why should you trust All Safe anyways?\n");
    } else {
...
int main( int argc, char** argv ) {
  ...
  if (ptrace(PTRACE_TRACEME, 0, 1, 0) < 0) {
      char nope[] = "\xa1\x5a\xfb\x0e\xe6\x21";
      decrypt_string(nope,7);
      printf(nope);
  }
